...
classdef GP < handle
    ...
    properties(SetAccess=private)
        ...
        M     % <n,n,p> length scale covariance 这个就是高斯核的超参数l
        var_f % <p>     signal/output covariance 这个是超参数det
        ...
        n % <1> input dimension X的维度
        p % <1> output dimension Y的维度
    ...
    methods
        function obj = GP(n, p, var_f, var_n, M, maxsize) %n:数据X的维度；p：数据Y的维度；M：这个就是高斯核的超参数l；maxsize:数据最大容许长度
        %------------------------------------------------------------------
        % GP constructor
        % args:
        %   n:       <1>     input dimension
        ...
        %   var_f:   <p>     signal/output covariance 这个是超参数det
        %   var_n:   <p>     evaluation noise covariance
        %   M:       <n,n,p> length scale covariance matrix这个就是高斯核的超参数l
        %   maxsize: <1>     maximum dictionary size
        %------------------------------------------------------------------
            obj.n       = n;   %GP模型输入（自变量zt个数）
            obj.p       = p;  %GP模型输出（因变量d个数）
            obj.X       = [];
            obj.Y       = [];
            obj.Nmax    = maxsize;    
        ...
        %------------------------------------------------------------------
            ...
            for pi = 1:obj.p
                D = pdist2(x1',x2','mahalanobis',obj.M(:,:,pi)).^2; %  马哈拉诺比斯距离:(xi-xj)(M-1)(xi-xj)'
        ...
        function add(obj, X, Y)  % 慕尼黑 eq(13)中的[xk,uk] [yk]
        %------------------------------------------------------------------
        ...
        %------------------------------------------------------------------
            ...
            % relevant points
                ...
                if strcmp(OPTION,'A')  % 比较函数，如果相同输出true，
                    for i=1:Nextra
        ...
        %------------------------------------------------------------------
            ...
            KxX = obj.K(x,obj.X);
            mu_y = zeros(obj.p,Nx);
            for pi=1:obj.p   %GP模型输出（因变量d个数）
                 mu_y(pi,:) = obj.mu(x) + KxX(:,:,pi) * obj.alpha(:,pi);   % 慕尼黑 KxX*(KXX)-1*y
            end
            ...
            for pi=1:obj.p
                for i=1:Nx
                    ...
                    v = obj.L(:,:,pi)\KxX(i,:,pi)';  % A=LL'  A-1=(L'-1)*(L-1)
                    K = obj.K(x(:,i),x(:,i));
                    var_y(pi,pi,i) = K(:,:,pi) - v'*v;   %慕尼黑 Kxx - KxX*(L-1)'(L-1)* KxX
                end
            end
        ...
        %------------------------------------------------------------------
            ...
            assert(obj.N>0, 'Dataset is empty. Aborting...') %检查是否有数据
            assert(obj.n==5, 'This function can only be used when dim(X)=2. Aborting...');  %可以处理两个维度的GP模型
            ...
            npoints = p.Results.npoints; %画图的点数
            sigmalevel = p.Results.sigmalevel;
            
            addParameter(p,'rangeX1', minmax(obj.X(1,:)) + [-1 1]*factor*range(obj.X(1,:)) ); %range返回矢量的最大值-最小值
            addParameter(p,'rangeX2', minmax(obj.X(2,:)) + [-1 1]*factor*range(obj.X(2,:)) );
            parse(p,varargin{:});
            
            rangeX1 = p.Results.rangeX1; % 向量X1的范围，扩展了30%
            ...
            Ytrue = zeros('like',X1);  %创建与X1类型相似的一个数
            Ystd  = zeros('like',X1);
            Ymean = zeros('like',X1);
            for i=1:size(X1,1)
                for j=1:size(X1,2)
            ...
            surf(X1,X2,Ytrue, 'FaceAlpha',.5, 'EdgeColor', 'none', 'DisplayName', 'True function');  %'FaceAlpha',.8 透明度为0.8
